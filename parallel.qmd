...
przypadku modeli, które w ramach jednego dopasowania wytwarzają wiele
podmodeli, funkcje `tune` potrafią to wykorzystać. Przykładowo model PLS (ang.
*Partial Least Squares*) jest podobny do zastosowania PCA na predyktorach, a
następnie przewidywania wartości wynikowej, z tą różnicą, że w PLS tworzone
składowe są dobierane tak, aby zmaksymalizować korelację pomiędzy nimi a zmienną
wynikową. Model PLS z 10 składowymi jest rozszerzeniem modelu z 9 składowymi —
wystarczy dopasować model z maksymalną liczbą składowych, a predykcje dla
mniejszej liczby składowych można uzyskać bez ponownego uczenia. Dzięki temu
dostrajanie liczby składowych w zakresie np. od 1 do 10 jest znacznie szybsze.
Taki mechanizm nazywamy optymalizacją podmodeli (ang. *submodel optimization*).
...
(*preprocessing*) są kosztowne obliczeniowo. Istnieją jednak dwa potencjalne
...
otrzymałyby do przeliczenia kilka foldów.
...
@fig-parallel2 ilustruje delegowanie zadań do procesów roboczych w tym schemacie; użyty
...
-   Istnieje pewna korzyść z używania `parallel_over = "everything"` z wieloma
...
czasu wykonania, który `parallel_over = "resamples"` osiąga przy użyciu tylko
...
Najlepsze przyspieszenie pracy, dla tych danych, występuje, gdy
...
Jaka jest korzyść z zastosowania metody optymalizacji podmodeli w połączeniu z
...
zastosowano optymalizację podmodeli). Dzięki optymalizacji
podmodeli i przetwarzaniu równoległemu uzyskano 282-krotny wzrost prędkości w
...
dostrajania ocenia wszystkie modele na początkowym podzbiorze foldów. W oparciu o
...
które mierzą spadek ROC AUC w stosunku do aktualnie najlepiej działających
parametrów, jak pokazano na @fig-parallel5.
...
możliwości wykorzystania predykcji podmodeli.
