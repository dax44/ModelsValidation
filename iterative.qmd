...
```{r}
purrr::map_dfr(
  list(cost = cost(), rbf_sigma = rbf_sigma()),
  ~ tibble(
    type = .x$type,
    range = paste0("[", paste(.x$range, collapse = ", "), "]"),
    transformer = if (is.null(.x$trans)) "-" else .x$trans$name,
    label = .x$label
  ),
  .id = "parameter"
)
...
górnym
rogu.](images/Zrzut%20ekranu%202023-03-9%20o%2011.49.58.png){#fig-iter1
...
Procedury wyszukiwania, które będziemy przedstawiać, wymagają przynajmniej kilku
...
parametrów. Funkcja `tune_grid()` ocenia tę siatkę z użyciem resamplingu:
...
gdzie $\sigma_{i,j}^2$ jest wariancją błędu modelu (tzw. *nugget*), równą $\sigma^2$ jeśli $i=j$ i zero w przeciwnym razie.
Możemy to interpretować jako, że wraz ze wzrostem odległości pomiędzy dwoma
kombinacjami parametrów, kowariancja pomiędzy metrykami wydajności maleje
...
-   Eksploatacja (ang. *exploitation*) - zasadniczo opiera się na istniejących
...
Zacieniowany obszar ufności odzwierciedla charakter funkcji kowariancji wykładniczej
...
(patrz @tbl-2). Rekomendacja parametru dostrajania dla 0,25 ma gorsze
...
```{r}
#| label: tbl-bo1
#| tbl-cap: Przebieg optymalizacji bayesowskiej
...
na różnych stronach granicy przestrzeni parametrów.
...
Punkt kandydujący jest oceniany przy zastosowaniu resamplingu, aby uzyskać
...
Proces ten trwa przez określoną liczbę iteracji, ale może zostać zatrzymany,
...
0,050. Używając promienia, który losowo waha się od 0,015 do 0,050, dane są
...
Przeszukiwanie metodą symulowanego wyżarzania może nie być optymalne w
...
wyżarzania](https://www.tmwr.org/sa_search.mp4){#fig-sa5
fig-align="center"}
