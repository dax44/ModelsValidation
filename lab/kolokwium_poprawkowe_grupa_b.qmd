---
title: "Kolokwium poprawkowe — grupa B"
...
- Wczytaj dane `Sacramento` i opisz ich podstawową strukturę: liczbę wierszy, kolumn, typy zmiennych.
...
- Na podstawie analizy opisz krótko, które zmienne mogą mieć największy związek z ceną domu i jakie potencjalne problemy mogą pojawić się przy modelowaniu (np. współliniowość, rozkłady skośne, duża liczba poziomów zmiennych nominalnych).
...
- **Reguła A** (MARS) — model samodzielnie wykrywający nieliniowości i interakcje:
  - Usunięcie predyktora o bardzo dużej liczbie poziomów (np. `zip`)
...
- Połącz każdy model wyłącznie z odpowiadającą mu regułą preprocessingu (MARS z regułą A, SVM z regułą B, las losowy z regułą C), tworząc trzy workflowy. **Nie krzyżuj** automatycznie każdego preprocessingu z każdym modelem.
