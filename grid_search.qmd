...
# Domyślne zakresy dla każdego parametru:
purrr::map_dfr(
  c("hidden_units", "penalty", "epochs"),
  function(nm) {
    p <- extract_parameter_dials(mlp_param, nm)
    tibble(
      parameter = nm,
      type = p$type,
      range = paste0("[", paste(p$range, collapse = ", "), "]"),
      transformer = if (is.null(p$trans)) "-" else p$trans$name
    )
  }
)
```
...
```{r}
#| label: fig-grid2
#| fig-cap: Siatka oparta na hipersześcianie łacińskim dla 20 kombinacji parametrów
set.seed(1303)
mlp_param %>% 
...
hipersześcianu łacińskiego. Ma on tendencję do tworzenia siatek, które dobrze
...
::: callout-tip
W `step_pca()`, użycie zerowej liczby komponentów PCA oznacza pominięcie
...
zakres przy użyciu planu hipersześcianu łacińskiego z 20 zestawami parametrów:
