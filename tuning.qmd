...
się oszacować na podstawie zbioru uczącego. Są też modele, których pewnych parametrów
nie szacuje się bezpośrednio podczas dopasowania modelu. Przykładem
...
losowym również są parametrami, których wartości nie szacuje się w procesie
...
hiperparametrów modelu nazywamy dostrajaniem modelu (ang. *tuning*). Definicję tę
można nawet rozszerzyć, jeśli pomyślimy o dostosowaniu takich hiperparametrów jak
...
oceny modelu — funkcji wiarygodności i dokładności (*accuracy*).
...
Biorąc pod uwagę powyższe wyniki, model z funkcją łączącą logit osiąga największą wartość log-wiarygodności.
...
#| cache: true
#| label: fig-tune1
#| fig-cap: Przedziały ufności dla funkcji wiarygodności
#| fig-align: center
set.seed(1201)
...
resampled_res <-
    ...
    summarize(
        ...
        std_err = sd(.estimate, na.rm = TRUE) / sqrt(sum(!is.na(.estimate))),
        .groups = "drop"
    )
...
mniejszym zbiorze danych; wartość uzyskana przez `broom::glance()` jest sumaryczną log-wiarygodnością,
podczas gdy `yardstick::mn_log_loss()` jest średnią stratą logarytmiczną na obserwację, a po zmianie znaku daje średnią log-wiarygodność na obserwację. Wyniki sugerują, że
wybór funkcji łączącej ma znaczenie i że model z linkiem logit wypada najlepiej dla tej metryki.
...
metrykami[^tuning-1], należy używać zestawów metryk, które dają pełniejszy obraz, oraz konsultować wybór metryk z odbiorcami wyników.
...
## Konsekwencje złego doboru hiperparametrów
...
warstwą wejściową i jedną warstwą ukrytą z sigmoidalną funkcją aktywacji.
Sieć neuronowa bez warstwy ukrytej (lub z liniową transformacją prowadzącą bezpośrednio do sigmoidy) jest równoważna regresji
logistycznej[^tuning-2]. Jednak już dodanie warstwy ukrytej z nieliniową aktywacją zwiększa złożoność modelu — wraz ze wzrostem liczby neuronów w warstwie
ukrytej rośnie zdolność do reprezentowania złożonych granic decyzyjnych.
...
treningowego. Jeśli ocena modelu jest dokonywana na podstawie ROC AUC
...
![Porównanie sieci neuronowych z różną liczbą neuronów w warstwie
...
wyspy wokół pojedynczych obserwacji, aby zminimalizować funkcję straty.
...
[^tuning-2]: dokładniej: sieć neuronowa bez warstwy ukrytej, z sigmoidalną funkcją aktywacji na wyjściu, jest równoważna regresji logistycznej
...
-   przeszukiwanie siatki - gdy wstępnie określamy zestaw wartości parametrów
    ...
    niemożliwa do opanowania. Z jednej strony ten fakt przemawiałby za niewybieraniem tej metody do poszukiwania optymalnych parametrów, ale jest to
...
W procesie dostrajania modelu możemy kalibrować zarówno hiperparametry główne modelu, jak i hiperparametry specyficzne dla danego silnika.
...
```{r}
parameters(hidden_units(), threshold()) |> as_tibble()
```
...
```{r}
parameters(spline_degree()) |> as_tibble()
```
...
extract_parameter_set_dials(ames_rec) %>%
    update(threshold = threshold(c(0.01, 0.10))) |>
    as_tibble()
```
...
Gdy receptura jest przygotowana, funkcja `finalize()` wykorzystuje dane treningowe do ustalenia górnej granicy
...
```{r}
rf_param |> as_tibble()
parameters(regularization_factor()) |> as_tibble()
```
...
tibble(
    type = penalty()$type,
    range = paste(penalty()$range, collapse = ", "),
    inclusive = paste(penalty()$inclusive, collapse = ", "),
    trans = penalty()$trans$name,
    label = penalty()$label
)
```
...
```{r}
parameters(penalty(trans = NULL, range = 10^c(-10, 0))) |> as_tibble()
```
