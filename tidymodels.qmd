...
regresja liniowa, może być budowany z wykorzystaniem różnych "silników": `stats`, `glm`, `glmnet`, `rstanarm` (korzystający z estymacji bayesowskiej).
...
To wszystko sprawiło, że autorzy pakietu `tidymodels` dokonali pewnego rodzaju standaryzacji modeli.
...
Choć regresję liniową możemy zbudować z wykorzystaniem 11 różnych pakietów, to my się ograniczymy tylko do `stats`, `glmnet` i `rstanarm`.
...
Już na poziomie definiowania modeli widzimy różnice w definicjach, np.
...
[^tidymodels-1]: korzysta z estymacji bayesowskiej pakietu `rstanarm`
...
Po ustaleniu modeli można je dopasować do danych, za pomocą funkcji `fit` w przypadku gdy określaliśmy zależność formułą lub `fit_xy` gdy zmienne niezależne i zależna były określone oddzielnie.
...
Unifikacja po pierwsze pozwala lepiej zapamiętać nazwy parametrów, a po drugie ich nazwy są zrozumiałe dla czytelnika, który niekoniecznie musi się znać na różnicach pomiędzy pakietami.
...
::: callout-caution
Nie należy wykonywać predykcji bezpośrednio na modelu bazowym, np. `predict(lm_form_fit$fit, ...)`.
Zamiast tego używaj `predict(lm_form_fit, new_data = ...)` lub predykcji z poziomu obiektu workflow.
Dlaczego? Funkcja predykcji modelu bazowego nie ma pojęcia, czy jakiekolwiek przekształcenia (np. z przepisu `recipes`) zostały dokonane na danych i pominie je, generując nieprawidłowe predykcje (czasami bez błędów).
:::
...
I o ile nie da się zunifikować wszystkich podsumowań modeli, ponieważ zawierają różne elementy, to w pakiecie `parsnip` korzysta się z funkcji `tidy` pakietu `broom` do podsumowania modelu.
...
Pakiet `workflows` zawiera szereg funkcji pozwalających skutecznie obsługiwać potoki `workflow`[^tidymodels-6].

[^tidymodels-6]: tak nazywa się funkcja do tworzenia potoku

Pomimo złożoności procedury modelowania można się dalej zastanawiać nad koniecznością stosowania przepływów, skoro można te czynności wykonywać oddzielnie.
...
PCA jest procedurą zależną od danych: kierunki głównych składowych są estymowane na zbiorze treningowym. Dlatego PCA powinna być włączona do przepływu modelowania, aby w trakcie resamplingu lub predykcji nowe dane były przekształcane przy użyciu parametrów wyznaczonych wyłącznie na danych treningowych, a nie szacowanych za każdym razem od nowa.

Choć `workflow` pozwalają na łączenie preprocessingu, tuningu i postprocessingu, to w następnym przykładzie pokażemy zastosowanie `workflow` do prostego uczenia modelu bez tych elementów.
...
Kolejną zaletą pakietu `workflowsets` jest możliwość jednoczesnego uczenia wielu wariantów modeli.
...
Jeszcze jedną wygodną funkcją do oceny ostatecznego modelu jest funkcja `last_fit`, której używamy dopiero po zakończeniu wyboru modelu i tuningu.
...
# predykcja na pierwszych pięciu obserwacjach
...
Wywołanie tej funkcji dokonałoby następujących czynności na predyktorach i zmiennej zależnej:

-   zmienna `Sale_Price` zostałaby przypisana jako zależna;
-   zmienna `Gr_Liv_Area` zostałaby przekształcona logarytmicznie;
-   zmienne `Neighborhood` i `Bldg_Type` zostałyby zamienione na indykatory stanów.
...
Jaka jest przewaga stosowania przepisów, nad prostą formułą lub surowymi predyktorami?
...
Czynności, które można wykonać przy użyciu kroków/przepisów jest bardzo wiele, od bardzo prostych, jak centrowanie i skalowanie (`step_normalize()`), po wyrafinowane, jak wybór spośród dni tygodnia tylko tych, które nie wypadały w święta (`step_holiday()`).
...
Aby temu zaradzić, można zastąpić brakujące wartości stałą wartością (`step_unknown()`).
Jeśli natomiast w nowych danych pojawi się poziom czynnika nieobecny w zbiorze treningowym, `step_novel()` przydzieli mu nowy poziom czynnika.
Rzadkie poziomy można z kolei połączyć w kategorię „inne" przy użyciu `step_other()`.
...
Jeśli dodamy `step_other(Neighborhood, threshold = 0.01)` do naszego przepisu, poziomy czynnika występujące w mniej niż 1% obserwacji[^tidymodels-9] zostaną wrzucone do nowego poziomu o nazwie „inne".
...
Jednak niektóre modele wymagają kodowania *one-hot encoding*, które tworzy indykatory wszystkich kategorii.
...
W pakiecie `recipes` istnieje oddzielny krok do tworzenia interakcji.
...
Zapis `+var1:var2` w formule modelu wymagałby znajomości kolumn przed wykonaniem `step_dummy()`, co nie ma miejsca w przepisach. W `recipes` interakcje tworzy się z kolumn istniejących na danym etapie przepisu — dlatego `step_dummy()` musi poprzedzać `step_interact()`, aby kolumny z prefiksem `Bldg_Type_` były już dostępne. Bez tego program nie miałby jak stworzyć interakcji.
...
[^tidymodels-11]: parametr `deg_free` oznacza stopnie swobody splajnu (złożoność), nie liczbę punktów węzłowych; większa wartość oznacza większą elastyczność

```{r}
#| label: fig-splajn
#| fig-cap: Opis zależności za pomocą splajnów z różną złożonością (stopnie swobody)
library(patchwork)
library(splines)
...
Rozwiązanie z 2 i 100 stopniami swobody wykazuje odpowiednio niedopasowanie i nadmierne dopasowanie.
Wybór 5 lub 20 stopni swobody wydaje się dużo lepszy.
...
Zauważmy, że wszystkie wspomniane kolumny są mierzone w stopach kwadratowych. Mimo że zmienne są w tych samych jednostkach, PCA jest wrażliwa na różnice wariancji, dlatego i tak zaleca się poprzedzenie jej `step_normalize()`.
