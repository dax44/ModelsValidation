...
Oceńmy rozkład różnic z prób bootstrapowych, powinien on oscylować wokół zera
zgodnie z hipotezą zerową.
...
null_bootstrap |>
    ...
    shade_ci(endpoints = ci) +
    geom_vline(
        xintercept = sample_diff$stat,
        linewidth = 2,
        color = "red"
    )
```

Widać wyraźnie, że próbkowa różnica median znacznie odbiega od rozkładu
zgodnego z hipotezą zerową, co jej przeczy.

```{r}
null_bootstrap |>
    get_p_value(obs_stat = sample_diff$stat, direction = "two-sided")
wilcox.test(x1, x2)
```
